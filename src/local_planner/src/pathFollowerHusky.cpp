...
int main(int argc, char** argv)
{
  ...
  // ros::Publisher pubSpeedHusky = nh.advertise<geometry_msgs::Twist> ("/husky_velocity_controller/cmd_vel", 5);
  ros::Publisher pubSpeedHusky = nh.advertise<geometry_msgs::Twist> ("/cmd_vel", 5);

  geometry_msgs::TwistStamped cmd_vel;
  geometry_msgs::Twist cmd_vel_husky;
  cmd_vel.header.frame_id = "vehicle";
  // cmd_vel.header.frame_id = "base_link";
  // cmd_vel_husky.header.frame_id = "vehicle";
    ...
    if (pathInit) {
      ...
      if (pubSkipCount < 0) {
        ...
        if (fabs(vehicleSpeed) <= maxAccel / 100.0) {
          ...
          cmd_vel_husky.linear.x = 0;
        } else {
          cmd_vel.twist.linear.x = vehicleSpeed;
          cmd_vel_husky.linear.x = vehicleSpeed;
        }
        cmd_vel.twist.angular.z = vehicleYawRate;
        cmd_vel_husky.angular.z = vehicleYawRate;
        pubSpeed.publish(cmd_vel);
        pubSpeedHusky.publish(cmd_vel_husky);
